...
export class RoleService {
    ...
    static compare(self: RoleResource, target: RoleResource) {
        ...
        return {
            ...
            isSameCurriculum:
                isSameYear &&
                (self.curriculum === target.curriculum ||
                    target.curriculum === "any"),
    ...
    static registrationKeyToRoleResource(
        registrationKey?: RegistrationKey & Class
    ...
    static studentToRoleResource(
        student?: StudentClassRole
    ): RoleResource | undefined {
        ...
        if (
            ...
            typeof student.class_number !== "number" ||
            student.grad_year === null ||
            student.curriculum_name === null
        ) {
    ...
    static _privilegeImpl(
        current: RoleResource | undefined,
        target: RoleResource | undefined
    ) {
        let privilege = {
            read: false,
            update: false,
        ...
        switch (current.role) {
            ...
            case StudentRole.Curriculum:
                ...
                privilege.grant =
                    compare.isSameCurriculum && compare.isAdminable;
                return privilege;
            case StudentRole.Year:
                // Prevent overriding the value of privilege.read by false
                privilege.read = privilege.read || compare.isSameYear;
                privilege.update = compare.isSameYear;
                privilege.delete = privilege.grant =
                    (compare.isSameYear && compare.isAdminable) ||
                    compare.isSame;
                return privilege;
    ...
    static privilegeSync(
        current: StudentClassRole | undefined,
        target: StudentClassRole | undefined
    ): Privilege;
    static privilegeSync(
        current: RoleResource | undefined,
        target: RoleResource | undefined
    ...
    static privilegeSync(...args: any[]): Privilege {
        ...
        if (
            !(
                args[0] instanceof RoleResource &&
                args[1] instanceof RoleResource
            )
        ) {
            [current, target] = [
                this.studentToRoleResource(args[0]),
                this.studentToRoleResource(args[1]),
            ];
        } else {
    ...
    static async privilege(...args: any[]): Promise<Privilege> {
        ...
        if (typeof args[0] === "number" && typeof args[1] === "number") {
            [current, target] = [
                this.studentToRoleResource(await StudentService.get(args[0])),
                this.studentToRoleResource(await StudentService.get(args[1])),
            ];
        } else if (
            args[0] instanceof RoleResource &&
            args[1] instanceof RoleResource
...
export const LocationService = {
    constructAmapURL: (keywords: string, city: string, page: number) => {
        return encodeURI(
            `https://restapi.amap.com/v3/place/text?key=${process.env.AMAP_SECRET}&keywords=${keywords}&types=高等院校&city=${city}&children=1&offset=20&page=${page}&extensions=all`
        );
    },
    consturctMapBoxURL: (keywords: string) => {
        return encodeURI(
            `https://api.mapbox.com/geocoding/v5/mapbox.places/${keywords}.json?access_token=${process.env.MAPBOX_TOKEN}&types=poi&limit=10`
        );
    },
        ...
        return axios
            ...
            .then((res) => {
                ...
                return res.data.pois.map((poi: any) => {
                    ...
                    try {
                        [longitude, latitude] = gcj02towgs84(
                            longitude,
                            latitude
                        );
                    } catch {
