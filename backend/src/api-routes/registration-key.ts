...
export const get: Operation = async (req, res) => {
    ...
    getSelf(req, res, logger).then((self) => {
        ...
        pg("wwg.registration_key")
            ...
            .select()
            .modify<(RegistrationKey & Class)[], (RegistrationKey & Class)[]>(
                (qb) => {
                    if (data["not_expired"]) {
                        qb.where(
                            "expiration_date",
            ...
            .then((result) => {
                logger.logComposed(self, Actions.access, "registration keys");
                sendSuccess(res, {
                    registration_keys: result.map<
                        Required<
                            RegistrationKeyInfo & {
...
export const post: Operation = async (req, res) => {
    ...
    getSelf(req, res, logger).then(async (self) => {
        ...
        const [classNumber, gradYear] = [
            data?.class_number ?? (self.class_number as number),
            data?.grad_year ?? self.grad_year,
        ];
        ...
        if (!!!class_) {
            ...
            sendError(
                res,
                400,
                `Cannot create a registration key for a class ${classNumber}, ${gradYear} that doesn't exist`
            );
            return;
        }
        ...
        const privilege = await RoleService.privilege(
            RoleService.studentToRoleResource(self),
            registrationKeyRole
        );
        ...
        if (!privilege.update) {
            ...
            sendError(
                res,
                403,
                "You are not allowed to create this registration key"
            );
            return;
        }
        ...
        const prefix =
            ...
            (expirationDate.getUTCMonth() + 1).toString().padStart(2, "0");
        expirationDate.setDate(expirationDate.getDate() + 7);

        pg("wwg.registration_key")
            .insert({
                registration_key:
                    prefix + randomBytes(16).toString("hex").slice(0, 8),
            ...
            .then((result) => {
                ...
                sendSuccess(res, {
                    registration_key: JSON.stringify(result[0]),
                });
            })
            .catch((err) => dbHandleError(err, res, logger.logger));
    });
    ...
    getSelf(req, res, logger).then(async (self) => {
        if (!!!data.expiration_date || !!!data.registration_key) {
            ...
            sendError(
                res,
                400,
                "Registration key and expiration date are required"
            );
            return;
        }
        ...
        try {
            ...
            const privilege = await RoleService.privilege(
                RoleService.studentToRoleResource(self),
                RoleService.registrationKeyToRoleResource(registrationKey)
            );
            if (!privilege.update) {
                logger.logComposed(
                    self,
                    ...
                    } (exp: ${registrationKey?.expiration_date.toISOString()}`,
                    true,
                    "the user didn't have enough privilege",
                    true,
                    data
                ...
                sendError(
                    res,
                    403,
                    "You are not allowed to update this registration key"
                );
                return;
            }
