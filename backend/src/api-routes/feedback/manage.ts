        ...
        pg("feedback")
            ...
            .then(async (result) => {
                let feedbacks = result.filter((feedback) => {
                    if (
                        feedback.class_number === null ||
                        feedback.grad_year === null
                    ) {
                    ...
                    const privilege = RoleService.privilegeSync(
                    ...
                    logger.logComposed(
                        ...
                        undefined,
                        false,
                        { privilege: privilege, feedback: feedback }
                    );
                ...
                const comments = await pg("comment")
                    ...
                    .whereIn(
                        "feedback_uid",
                        feedbacks.map((val) => val.feedback_uid)
                    );

                logger.logComposed(self, Actions.access, "feedbacks");
                sendSuccess(res, {
                    feedbacks: parseFeedbackComments(feedbacks, comments),
                });
            })
